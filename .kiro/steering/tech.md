# Website
- The `core-front` website uses the Express webserver.
- The site is written in TypeScript and Nunjucks is used the templating language for the web pages.
...
- The base template for `core-front` is `ipv-core-base.njk` from the `@govuk-one-login/frontend-ui` npm package.
  ...
  - This template uses variables set as res.locals in core-front (e.g. in `src/lib/locals.ts`), so when investigating whether a local variable is used, always check this base template as well as core-front's own templates.
