...
When(
  /^(?:user selects|I select) (?:I am |they are )?from the UK$/,
  async ({ identityPage }) => {
...
When(
  "user enters data in address stub and clicks on submit data and generate auth code",
  async ({ addressPage, page }) => {
    ...
    if (currentUrl.includes("pyi-no-match")) {
      throw new Error(
        "Passport submission resulted in pyi-no-match error page. The passport credential data may be invalid or not configured for this journey type.",
      );
    }
...
When(
  "user enters data in fraud build stub and clicks on submit data and generate auth code",
  async ({ fraudPage, page }) => {
    const currentUrl = page.url();
...
Then(
  "the user should land onto the PIP page and select {string} and continue",
  async ({ page }, choice: string) => {
    ...
    expect(normalise((h1Text || "").trim())).toBe(
      "Tell us if you get Personal Independence Payment (PIP)",
    );
    ...
    const continueButton = page
      .locator(
        '#continue, #submitButton, button[type="submit"], button:has-text("Continue")',
      )
      .first();
