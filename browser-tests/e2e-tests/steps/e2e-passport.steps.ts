...
When(
  "user starts a fresh full journey in {string}",
  async ({ orchestratorPage }, _env: string) => {
...
When(
  "the {string} feature set is enabled",
  async ({ identityPage }, featureSet: string) => {
...
When(
  ...
  async ({ passportPage, page }) => {
    await passportPage.selectTestDataAndSubmit(
    ...
    if (afterUrl.includes("pyi-no-match")) {
      console.error(
        "[E2E Passport] Passport submission led to pyi-no-match error page",
      );
    }
  },
);
...
Then("user should be on Fraud Check \\(Stub)", async ({ page }) => {
  await expect(page.locator('h1, [role="heading"]').first()).toContainText(
    "Fraud Check",
    { timeout: 10000 },
  );
});
...
Then("User should be on KBV page and click continue", async ({ page }) => {
  ...
  const continueButton = page
    .locator('#submitButton, #continue, button[type="submit"]')
    .first();
  await continueButton.click();
});

When(
  "user enters data in kbv stub and clicks on submit data and generate auth code",
  async ({ kbvPage }) => {
    await kbvPage.fillAndSubmit(
...
Then(
  "'vot' should be set to {string} in the raw user info object",
  async ({ page }, votValue: string) => {
    await expect(page.locator("#main-content").first()).toContainText(
      votValue,
      { timeout: 10000 },
    );
  },
...
When(
  "user starts a new full journey with the same userId in {string}",
  async ({ orchestratorPage }, _env: string) => {
...
Then(
  "the user should be taken to the IPV Reuse Screen with One login changes",
  async ({ identityPage }) => {
...
Then("Relevant changes for VOT displayed in JSONResponse", async ({ page }) => {
  ...
  await expect(
    page.locator("span").filter({ hasText: "Raw User Info Object" }).first(),
