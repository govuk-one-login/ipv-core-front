...
When(
  "I confirm I do not have photo ID suitable for an app journey",
  async ({ identityPage }) => {
...
Then(
  "I should see page with heading {string}",
...
When(
  "I enter Face to Face Stub data for valid passport and submit",
  async ({ f2fPage }) => {
...
Then(
  "I should see go to post office page without VC",
  async ({ identityPage }) => {
  ...
  async ({ page, orchestratorPage, identityPage }) => {
    ...
    while (Date.now() - startTime < maxWaitMs) {
      ...
      try {
        ...
        // Success!
        console.log(
          `[F2F Step] ✅ Kenneth screen found at ${Date.now() - startTime}ms!`,
        );
