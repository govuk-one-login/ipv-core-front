...
When(
  "clicks {string} radio, then continue",
  async ({ page }, value: string) => {
    ...
    if (await radioByValue.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const escapedValue = value
        .replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
        .replace(/'/g, "['\u2019]");
      const labelByText = page.locator("label", {
        hasText: new RegExp(escapedValue),
      });
...
When(
  "submits {string} evidence with scores",
  async ({ page }, name: string, dataTable: any) => {
    ...
    if (scores.activity)
      await page
        .locator("#activity, #activityHistory")
        .first()
        .fill(scores.activity);
...
Then("user should have a {string} identity", async ({ page }, vot: string) => {
  ...
  await page.waitForURL((url: URL) => url.toString() !== currentUrl, {
    timeout: 30000,
  });
