...
test.describe.parallel("Accessibility tests", () => {
  ...
  iteratePagesAndContexts((pageName, context, language, url) => {
    test(`Accessibility check for ${pageName}, context ${context} and language ${language}`, async ({
      ...
      const allSummaryDetails = await page
  ...
  ["radio", "checkbox"].forEach((errorState) =>
    ["en", "cy"].forEach((language) =>
      test(`Accessibility check for ${errorState} error state in ${language}`, async ({
        page,
      }) => {
        await page.goto(
          `${domainUrl}/dev/template/confirm-your-details/${language}?errorState=${errorState}`,
        );
        await assertNoAccessibilityViolations(page);
      }),
    ),
