...
import { buildCredentialIssuerRedirectURL, redirectToAuthorize } from "../shared/criHelper";

import { generateAxiosConfig, generateAxiosConfigWithClientSessionId } from "../shared/axiosHelper"
import { logError, logCoreBackCall, transformError } from "../shared/loggerHelper"
import { LOG_COMMUNICATION_TYPE_REQUEST, LOG_TYPE_JOURNEY, LOG_COMMUNICATION_TYPE_RESPONSE, LOG_TYPE_CRI, LOG_TYPE_CLIENT, LOG_TYPE_PAGE } from "../shared/loggerConstants";
import { samplePersistedUserDetails, generateUserDetails } from "../shared/reuseHelper";
import HTTP_STATUS_CODES from "../../appConstants";
import axios from "axios";
import { getIpAddress } from "../shared/ipAddressHelper";
import fs from "fs";
import path from "path";
import saveSessionAndRedirect from "../shared/redirectHelper";

const { API_BASE_URL, ENABLE_PREVIEW, API_BUILD_PROVEN_USER_IDENTITY_DETAILS } = config;

async function journeyApi(action: string, req: { session: { clientOauthSessionId: any; }; }) {
...
export async function handleJourneyResponse(req: any, res: any, action: string): Promise<any> {
  const backendResponse = (await journeyApi(action, req)).data;
  return await handleBackendResponse(req, res, backendResponse);
}

export async function handleBackendResponse(req: any, res: any, backendResponse: any) {
...
function tryValidateCriResponse(criResponse: { redirectUrl: any; }) {
...
function tryValidateClientResponse(client: { redirectUrl: any; }) {
...
export async function renderAttemptRecoveryPage(req: any, res: any): Promise<any> {
  res.render("ipv/pyi-attempt-recovery.njk", {
    csrfToken: req.csrfToken(),
  });
}

export async function updateJourneyState(req: any, res: any, next: NextFunction):Promise<any> {
  try {
...
export async function handleJourneyPage(req: any, res: any, next: any) {
  try {
...
export async function handleJourneyAction (req: any, res: any, next: any) {
  try {
...
export async function handleMultipleDocCheck(req: any, res: any, next: NextFunction) {
  try {
...
export async function handleCriEscapeAction(req: any, res: any, next: NextFunction) {
  try {
    if (!req.session?.ipvSessionId) {
      const err = new Error("req.ipvSessionId is missing");
      err.status = HTTP_STATUS_CODES.UNAUTHORIZED;
...
export async function handleCimitEscapeAction(req: any, res: any, next: NextFunction) {
  try {
    if (!req.session?.ipvSessionId) {
      const err = new Error("req.ipvSessionId is missing");
      err.status = HTTP_STATUS_CODES.UNAUTHORIZED;
...
export async function renderFeatureSetPage (req: any, res: any) {
  res.render("ipv/page-featureset.njk", {
    featureSet: req.session.featureSet,
  });
};

export async function allTemplates (req: any, res: any, next: NextFunction) {
  try {
...
export async function validateFeatureSet(req: any, res: any, next: NextFunction) {
  try {
