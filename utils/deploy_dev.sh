...
function clone_config_repo() {
  [ -d config-repo ] || git clone --no-checkout --depth=1 --sparse --quiet git@github.com:alphagov/di-ipv-config.git config-repo

  list_of_developers="core/ci/core-developer-pipelines/generate-pipelines/list_of_developers.txt"
  environments=($(cd config-repo || print_error_exit "Cannot find config-repo"; git pull --depth=1 -q; git show HEAD:"${list_of_developers}"))
}
